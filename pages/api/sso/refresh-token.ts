...
export default function handle(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        const token = jwt.sign({ username: payload.username }, process.env.JWT_ACCESS_TOKEN_SECRET_KEY, { expiresIn: '15m' });
        const newRefreshToken = jwt.sign({ username: payload.username }, process.env.JWT_REFRESH_TOKEN_SECRET_KEY, { expiresIn: '7d' });
